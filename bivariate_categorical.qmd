...
Also known as cross-tabulations---or crosstabs for short---contingency tables are quite similar to contingency tables. Rather than showing the distribution of a single variable, they show us the distribution of our dependent variable across the levels of our independent variable.
...
This would be our alternative hypothesis, and its respective null hypothesis is that Democrats & Republicans are equally likely to approve of the death penalty.

In other words, the alternative hypothesis is that the proportion of approval for the death penalty will be different across political parties. The null hypothesis is that there will be no such difference.
...
If you did not save `our_gss.rda` after creating this variable in a previous exercise, you can go ahead and recreate it with the following code:
...
So, the observed value is easy enough---it's just the value we actually see in our contingency table. So, for example, in the contingency table we have here:
...
The observed value of the cell in the first row of the first column is 426. Now, this raises the question of how exactly we calculate the expected values.
...
This calculation is actually a slight variation on a principle from probability theory that describes the joint probability of independent events: $P(A\&B) = P(A)\cdot P(B)$

What this means is that, if two variables are independent, then the probability of any combination of these variables occurring simultaneously is equivalent to the product of each event's individual probability.

So, consider an example from our data:

The $P(Democrat\&Favor) = P(Democrat)\cdot P(Favor)$

The probability of favoring the death penalty and being a democrat is equal to the probability of being a democrat multiplied by the probability of favoring the death penalty.
...
Now, all we'd need to do is plus these expected values into this formula: $\chi^{2} = {\sum}{\frac{(o - e)^{2}}{e}}$

Then you can use a look-up table for the corresponding p-values of Chi-squared statistics. You will need to know your degrees of freedom, which, for the Chi-square test, is $(RowNumbers-1)\cdot (ColumnNumbers-1)$. So, for our 2x2 contingency table, this would be:
...
You can then use a look-up table for p-values that correspond with a given Chi-square test value. [Here's an example from the University of Sussex](https://users.sussex.ac.uk/~grahamh/RM1web/ChiSquareTable2005.pdf).

All you have to do is take your Chi-squared value & your degrees of freedom and then locate the column corresponding to the alpha level you are hoping for. In most cases, this will be an alpha of 0.05. We want a p-value below this alpha level for statistical significance, which means that we want less than a 5% chance of a false positive (i.e. returning a statistically significant result when there actually is no association).

With an alpha of 0.05 and 1 degree of freedom, we need our Chi-squared value to be greater than 3.84 in order to claim statistical significance. In our case, we do have this, and I'll show you a much simpler way of sussing all this out below.
...
Thankfully for all of us, you will rarely if ever need to actually go about manually calculating all of this stuff. As usual, R has a nifty function that we can run directly on our contingency table, which will automatically run all of these calculations and output a corresponding p-value. :::{callout-caution} Note here that I am running this on a bare-bones table without any of the `adorn_()` functions that we have used to dress up our `tabyl()` output. Because this adds a bunch more information beyond the simple frequency counts, this will throw off `chisq.test()`, which will not know what to do with the extra info (e.g. the percents & totals row). Make sure you leave out any adornments when running the Chi-square test. :::
...
This gives us our Chi-squared value (302.18), our degrees of freedom (1), and a corresponding p-value. Note that this is scientific notation, but it's an extraordinarily small number---far below our 0.05 alpha threshold. I'll display it in full so you can see for yourself
...
In the case that it is this small, it's common convention to simply report that your p-value is \< 0.001.
...
As you can see, far fewer Democrats favor the death penalty than would be expected in the case that this is independent from one's political affiliation. Similarly, many more Republicans favor the death penalty that we would expect in the conjoint distribution of two independent variables.

So, what does this mean substantively?

With a Chi-squared value of 302.18 at 1 degree of freedom, we find evidence to support our hypothesis that there is an association between death-penalty attitudes and political-party affiliation. Our p-value of \<0.001 suggests it is exceedingly unlikely that the relationship we observe in our cross-tabulation is the result of random chance or peculiarities of our sample.
