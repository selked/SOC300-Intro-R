...
:::{.callout-note}
Remember that when you see the `#` symbol in R, that indicates the following text should be treated as a comment for us rather than something for R to process. Note that I use some descriptive comments below to help explain what the code is doing
:::

```{r}
our_df<-data.frame(
  p_id = p_id_vec, # a participant ID variable for each dog owner
  dog_size = ordinal_vec, # a variable for the number of dogs each participant has
  side_of_town = character_vec, # the side of town each participant lives in
  food_per_day = num_vec, # daily dog food amount in ounces
...
our_df$dog_size <- factor(
  our_df$dog_size, # the first input is the vector we are turning into a factor
  levels=c("small", "medium", "large"), # then we indicate the levels in the order that makes sense
