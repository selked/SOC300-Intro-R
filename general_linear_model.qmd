...
In [Chapter 6](https://selked.github.io/SOC300-Intro-R/bivariate_categorical.html), we explored some common techniques for displaying and interpreting the bivariate distribution when all our variables are categorical.
...
The math behind this can really get into the weeds, but I'll give a sense of how this line is derived. The aim is minimize a value called the **sum of squared residuals (SSR)**. Residuals are the difference between a regression and the observed data points. The estimating procedure behind OLS involves using our actual $X$ and $Y$ values to define a range of possible lines, and the SSR is calculated for each of them. The line associated with the lowest SSR is the one taken as the best-fitting representation of the linear association between $X$ and $Y$. This is also how we get the term 'least squares' for this style of regression.
...
mockdat <- workingjobs.df

names(mockdat)[144] <- "Percentage Women"
names(mockdat)[141] <- "Warmth/Competence Scale"
```
...
c_tab <- mockdat |>
  lm(`Warmth/Competence Scale` ~ `Percentage Women`, data = _) |>
  tbl_regression(
    estimate_fun = label_style_sigfig(digits = 3),
    intercept = TRUE) |>
  modify_header(label ~ "**Variable**") |>
  add_glance_table(include = "r.squared") |>
  modify_caption(caption = "<div style='color: white; font-weight: bold'>OLS Regression Results</div>")
...
```{r, eval = TRUE, echo = FALSE}
c_tab <- mockdat |>
  lm(`Warmth/Competence Scale` ~ `Salary`, data = _) |>
  tbl_regression(
    estimate_fun = label_style_sigfig(digits = 3),
    intercept = TRUE) |>
  modify_header(label ~ "**Variable**") |>
...
workingjobs.df |>
  ...
  summarize(
    "Mean (Lower middle class)" = mean(comp_sim)
  ) |>
  knitr::kable()
...
workingjobs.df |>
  ...
  summarize(
    "Mean (Middle Class)" = mean(comp_sim)
  ) |>
  knitr::kable()
...
c_tab <- mockdat |>
  lm(`Warmth/Competence Scale` ~ `Percentage Women` + `Salary`, data = _) |>
  tbl_regression(intercept = TRUE, estimate_fun = label_style_sigfig(digits = 3)) |>
  modify_header(label ~ "**Variable**") |>
  modify_caption(caption = "<div style='color: white; font-weight: bold'> OLS Regression Coefficients</div>")
...
And finally, I will plot the coefficients themselves, so you can see more clearly how the association is influenced by controlling for salary. What follows is the delightfully named 'dot-and-whisker' plot. In our case, the x-axis tracks the slope estimates of our two models. The y-axis tracks the different slope estimates from our model that we're interested in plotting. In our case, we are just looking at the percentage of women, so we only have one variable. We will see two estimates plotted along the y-axis, because we are looking at estimates of Percentage Women for two different models---one with no controls and one controlling for salary.
...
The 'dot' in our dot-and-whisker plot is the value of the slope coefficient estimated by each model. The 'whiskers' are the thin little tails on either side of the dot. These reflect the **standard errors**. The interpretation of these can get a little tricky, so I'll stay conceptual again. Our model predicts a specific slope coefficient, but it also uses information from our data to estimate the degree of uncertainty surrounding this specific estimation. The standard error, in technical terms, is the standard deviation of the estimated slope coefficient of our sample. In other words, the specific **point estimate** is the model's best estimation, and the standard error gives a picture of the slightly broader range of values in which we can be confident that the true population value will be found.

These plots very succinctly capture differences in slope coefficients for different variables in a regression analysis, and they are also very simple to interpret when it comes to statistical significance. Because a slope of 0 indicates no linear relationship, any slope estimate whose whiskers (i.e range of standard error) do not cross '0' on the X-axis are statistically significant.
