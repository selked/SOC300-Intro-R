...
our_tab
```
We can actually do the same thing with the rownames as well. In this case, we could capitalize 'favor' and 'oppose' and leave the rest as is.
```{r}
# Check the row names 
rownames(our_tab)

# Overwrite the row names with our own
rownames(our_tab) <- c("Favor", "Oppose", "NA", "Total")
```
