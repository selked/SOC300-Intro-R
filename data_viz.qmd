...
our_tab <- our_gss |>
  ...
  adorn_rounding(digits = 2)
```
This table has all the right info we need, but it would be nicer if we didn't have the 'cappun' shorthand for the variable name. The other columns could be capitalized, and we could also stand to lose the underscore in 'valid_percent'. 
...
```
Good stuff. I'll point out that we use the `~` symbol above, but don't worry too much about that. It's one of the pecularities I alluded to about `gtsummary`. Just know that we have to use `~` there. Otherwise, there's lots of great detail here, so this is pretty much good to go.
