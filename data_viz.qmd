...
In this last portion of our in-class lab sessions on quantitative analysis, we'll learn how to make some familiar visuals in new and exciting ways. For the most part, this will involve learning the graphing package of tidyverse, commonly known as `ggplot`, as well as `gt` and `gtsummary`, two helpful packages for producing tables. There are a couple of things to get used to with ggplot, but once you get a handle on its syntax, it will offer simple and extensive customizability that translates across a wide expanse of use cases. Even some of the graphics that we've seen in empirical papers for class have been produced using ggplot! 
...
The structure of figures made with ggplot is quite regular. The name 'ggplot' is actually a reference to the broader idea of a **g**rammar of **g**raphics, and though there are some specifics across the various plot types, we will offer ggplot mostly the same kind of information regardless. It's certainly not necessary, but if you are curious, you can [read some more about the grammar of graphics here](https://info5940.infosci.cornell.edu/notes/dataviz/grammar-of-graphics/)
...
I'll load them in below. If you need to install them, you can run `install.packages("gtsummary")`
`install.packages("gt")`
...
```
So, for `tbl_cross()` we provide our row variable and our column variable. If we  set `percent = "column"`, we will get percentages for each cell that will total with respect to the the columns---this is what we did with our `tabyl()` output, as it allows for more intuitive interpretation of the relationship between our variables. For `margin = c("column", "row")`, we're telling the function that we want totals for both columns and rows. Lastly, we give some better labels for our variables. The `labels =` input is designed to work for multiple variables, so it expects that we will give it a `list()` object. Even if you only need a label for one variable, you will need to put it inside the `list()` function.
