...
There's a nifty function called `tbl_cross()` which basically does the same thing as `tbl_summary()` but is more specific to contingency tables.

Remember to always provide your dependent variable to the 'rows' input.
...
```
So, for `tbl_cross()` we provide our row variable and our column variable. If we had `percent = "cell"`, each cell will include a percentage in addition to a count. For `margin = c("column", "row")`, we're telling the function that we want totals for both columns and rows. Lastly, we give some better labels for our variables.
...
```
This is starting to sprawl a little, but it's basically the same command as before, except that now we have put our `tbl_cross()` command inside of the `tbl_strata()` function. Here's the `tbl_strata()` function as it's used in our code (note that the parentheses isn't closed until the very bottom of our command up above):
...
So, the 'strata' input is where we indicate our test variable. The effect of `strata = sex` is somewhat similar to `group_by()` from tidyverse---it groups the data by our test variable.
...
.tbl_fun is shorthand for "table function", and `.x` is an R-specific placeholder for function inputs. Essentially, we are saying "group the data by our `strata` variable, and then pass that stratified data frame into another function of my choosing." 

Then, we can just give our `tbl_cross()` function from before, and this will be calculated according with the grouping structure of the test variable in mind.
